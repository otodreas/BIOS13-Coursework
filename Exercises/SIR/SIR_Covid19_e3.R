...
SIRV_func <- function(t, SIRV, P) {
  ...
  if (t<P$t_vaccine) { #before vaccination the model as before
    ...
    dIdt <- P$beta * I * S - P$rho * I + (1-P$vaccine_efficiency)*P$beta*I*V
    dRdt <- P$rho * I - P$gamma*R - P$c*R
    dVdt <- P$c*(S+R) - (1-P$vaccine_efficiency)*P$beta*I*V
  }
...
# Assume that vaccination came into place after one year
# Assume that efficiency < 100% gives infection of the vaccinated population 

P <- list(beta=beta, rho=rho, gamma=1/180, c=1/30, t_vaccine=365, vaccine_efficiency=0.5)
